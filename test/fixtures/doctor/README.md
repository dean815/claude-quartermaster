...
| `hook-entry-ignored` | `hooks: { PreToolUse: 42 }` | error, `This entry was ignored.` | `true` | the third partial-acceptance unit: one **entry of a record** |
...
`hook-entry-ignored` is the state that could not be recorded until it could, and then
stopped being that state. DEA-151 argued `unknown` is the safe default for a message from
neither family and noted it had no first-party instance, so its fixture had to be
constructed. 2.1.224 supplied one: a malformed hook *event* prints `This entry was
ignored.` — one word off the sentence `FIELD_IGNORED_NOTE` pins — and `enabled: true` says
Claude Code applied the file anyway. It landed in `not-checked` **carrying schema errors**,
which was the visible half of that trade.

**DEA-152 took the decision the recording made arguable**, and it is the cost unit that
took the work: an entry of a record is neither of DEA-149's two models. So `costs` has a
third value, `entry`, and the finding names the key and prints **no number** — what a
dropped hook event or a dropped marketplace decides has never been measured, and a figure
whose unit nobody established is the generalisation DEA-147 had to correct. The recording
is unchanged; this release's reading of it is what moved.

The cost of that is paid on this same fixture set: **no recording produces `unknown` any
more**, so the state DEA-151 exists for is back to being constructed, in
`test/dropped-field.test.ts`. Teaching the classifier a message is also spending the only
live evidence that its default works.
...
- `extraKnownMarketplaces.<id>.source` as a string — the DEA-147 **incident key** — voided
  ...
  in a third phrasing. `discarded-marketplace-source` and `discarded-many-entries` remain
  honest recordings of the release named on them — the same key, refused, on the release
  that refused it.
- `hooks.PreToolUse` gained the `This entry was ignored.` phrasing, recorded above.

Both landed in `not-checked` when recorded, which is the direction DEA-151 chose and the
reason neither one produced a wrong finding — and both are recognised as of DEA-152, as
`entry`. So one key in this set is recorded twice with **opposite** verdicts, two releases
apart, and both recordings are correct. That is the argument for recording rather than
re-probing, stated by the fixture set itself.
...
- The failure classes are told apart by matching the message line against lists of
  first-party phrasings in `src/delegate/doctor.ts` — three that keep the file
  (`IGNORES_THE_FIELD`, `IGNORES_THE_ENTRY`, `REMOVES_ARRAY_ELEMENTS`, which differ by what
  the error *cost*) and one that refuses it (`REFUSES_THE_FILE`). All are open sets of
  Claude Code's own prose with no version guarantee behind them, so a message matching none
  is `not-checked` and never `discarded`; which way that errs, and why it errs that way, is
  argued at `costOf`.
