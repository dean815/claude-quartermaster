...
**Re-summarize N · ~N min** runs the script after a confirmation.

Measured, not estimated: a run over 10 stale sessions took **147 seconds**, so
budget about 15 seconds a session. It reads far more than it writes — every
excerpt goes in to get three lines out.

Deliberately not priced in dollars. Auth is an OAuth subscription, so a run
draws down rate-limit headroom and bills nothing; the CLI's `total_cost_usd` is
what those tokens *would* have cost at API rates, which is not the same thing.
...
| `resummarize.sh` | Headless `claude -p` run of the skill — the only part that needs a model |
